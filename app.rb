...
post '/:user/new_item' do
    params[:tempfile] = params[:link]
    img = Imgur::LocalImage.new(params[:tempfile], title: 'generic')
    @uploaded = imgurClient.upload(img)
    puts @uploaded.link
    User.find(params[:user]).image_things.create(description: params[:imagey], due: params[:date], image: @uploaded.link)
